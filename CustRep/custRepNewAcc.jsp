...
<body>
	...
	<%
		if("POST".equalsIgnoreCase(request.getMethod())){
            ...
            ResultSet rs;
            rs = stmt.executeQuery("select * from custrep where username='" + username + "'");
            if (rs.next()) {
    %>
            	<p style="color: green;">Username exists!!.</p>
    <% 
            	out.println("Please login using this link <a href='custRepLogin.jsp'>Login</a>");
            } else {
            	int x = stmt.executeUpdate("insert into custrep (username, password) values('" + username + "', '" + password + "')");
            	con.close();
